...
class QuantizableResNet(ResNet):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.quant = torch.ao.quantization.stubs.QuantStub()
        self.dequant = torch.ao.quantization.stubs.DeQuantStub()

    def forward(self, x: Tensor) -> Tensor:
        x = self.quant(x)
        x1 = x
        #print('x = ', torch.int_repr(x))
        # uint8_ndarry = torch.int_repr(x).numpy()
        # print('x = ', uint8_ndarry)
